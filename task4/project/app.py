...
async def start():
    ...
    # The actual summarization logic is handled by the agent's instructions
    # and the implicit call to SummarizerTool by the LLM
    ...
    async for event in result.stream_events():
        if event.type == "raw_response_event" and isinstance(event.data, ResponseTextDeltaEvent):
            ...
            if event.item.type == "tool_call_item":
                # Assuming the agent will call SummarizerTool implicitly
                # We can add a message here if we want to explicitly show tool calls
                pass
            elif event.item.type == "tool_call_output_item":
                # This will contain the output of the tool if it was explicitly called
                pass
...
async def handle_quiz_request(action: cl.Action = None):
    ...
    # The agent will implicitly call QuizGeneratorTool
    ...
    async for event in result.stream_events():
        if event.type == "raw_response_event" and isinstance(event.data, ResponseTextDeltaEvent):
            ...
            if event.item.type == "tool_call_item":
                # Assuming the agent will call QuizGeneratorTool implicitly
                pass
            elif event.item.type == "tool_call_output_item":
                pass
